...
import org.example.config.SocketHandler;
...
//        Thread.sleep(1000);
        return CommonResponse.success(SocketHandler.SocketSessionDataBase.stream().filter(item -> !item.getId().equals(sessionId)).collect(Collectors.toList()));
    }
    ...
    public CommonResponse<List> createPolicy(@PathVariable String senderId, @PathVariable String receiverId) {
        return CommonResponse.success(
                SocketHandler.messageDataBase.stream()
